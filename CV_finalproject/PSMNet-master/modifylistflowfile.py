...
def dataloader(filepath):
  ...
  for ss in subdir:
    ...
    for ff in flying:
      ...
      imm_r = os.listdir(flying_dir+ss+'/'+ff+'/right/')
      imm_d = os.listdir(flying_disp+'/TRAIN/'+ss+'/')
      for im in imm_l:
       if is_image_file(flying_dir+ss+'/'+ff+'/left/'+im):
         all_left_img.append(flying_dir+ss+'/'+ff+'/left/'+im)
      for im in imm_d:
       all_left_disp.append(flying_disp+'/TRAIN/'+im)
      for im in imm_r:
  ...
  for ss in subdir:
    ...
    for ff in flying:
      ...
      for im in imm_d:
       test_left_disp.append(flying_disp+'/TRAIN/'+im)
      for im in imm_r:
       if is_image_file(flying_dir+ss+'/'+ff+'/right/'+im):
